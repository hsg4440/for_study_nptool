...
void FillHistoE()
{
  ...
  // Output file
  TFile * ofile = new TFile("backE_histo_file_run57.root","recreate");
  ...
  for(int k=0; k<number_of_detectors; k++){
    ...
    TString histo_name2 = Form("cinematique_h2_det%i",k+1);
    h2[k] = new TH2F(histo_name2,histo_name2,500,0,60,500,0,200);
  }
  ...
  for(int i=0; i<nentries; i++){
    ...
    if(mult==1){
      ...
      TVector3 PosTarget = TVector3(-0.98, -0.4, 0);
      TVector3 pos = PosPista - PosTarget;
      double theta  = pos.Theta()/TMath::Pi()*180;

      double Elab_th = g1->Eval(theta);
      ...
      double Ecal = p0 + p1*E_back;
      double Elab = DE_strip+Ecal;
      double Ex = r1->ReconstructRelativistic(Elab,theta*TMath::Pi()/180.);
     
      int condition = cut[det-1]->IsInside(E_back,DE_strip);
      
      if(det>0 && det<9 && stripE>0 && stripE<58 && stripDE>0 && stripDE<92 && E_back>18000 && fTS_TMW==0 && condition && Ex<2.5 && Ex>-2){
        h[det-1]->Fill(E_back,Eres);
        h2[det-1]->Fill(theta,Elab);
      }
    }
  }
  ...
  ofstream ofile1;
  string ofilename1 = "PISTA_BACK_E_run57.cal";
