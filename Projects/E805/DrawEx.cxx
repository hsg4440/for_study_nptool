...
bool TestEntry() {
  ...
  for (unsigned int i = 0; i < TACPhysics.TAC_Time.size(); i++) {
    // if(TACPhysics.TAC_Name[i].compare("TAC_CATS_PL") == 0 && abs((int)TACPhysics.TAC_Time[i] - 28000) < 1500 &&
    // ZDDPhysics.PL_TS.size() > 0 && abs((int)(TACPhysics.TAC_TS[i] - ZDDPhysics.PL_TS[0]) -61) <4)
    if (TACPhysics.TAC_Name[i].compare("TAC_CATS_PL") == 0 &&
        cut_Cr->IsInside(TACPhysics.TAC_Time[i], ZDDPhysics.ICSum) && ZDDPhysics.PL_TS.size() > 0)
      TACCondition1 = true;
  }
  if (!TACCondition1)
    return false;
...
void DrawEx() {
  ...
  std::map<unsigned int, TH2F*> hExd2_WithCATSs_m;
  ...
  for (unsigned int i = 1; i <= 4; i++) {
    ...
    hExd2_WithCATSs_m[i] = new TH2F(Form("hExd2_ws_%i", i), Form("hExd2_ws_%i", i), 200, 10, 30, 450, -5, 25);
    hExd_NoCATS_m[i] = new TH1F(Form("hExd_n_%i", i), Form("hExd_n_%i", i), 450, -5, 25);
    hELTLd_s_m[i] = new TH2F(Form("hELTLd_s_%i", i), Form("hELTLd_s_%i", i), 200, 0, 40, 250, 0, 200);
    hExTLd_m[i] = new TH2F(Form("hExTLd_%i", i), Form("hExTLd_%i", i), 200, 0, 40, 200, -5, 15);
    hExt_m[i] = new TH1F(Form("hExt_%i", i), Form("hExt_%i", i), 450, -5, 25);
    hExd_WithCATSs_CSI[i] = new std::map<unsigned int, TH1F*>;
    for (unsigned int j = 1; j <= 16; j++) {
      (*hExd_WithCATSs_CSI[i])[j] =
          new TH1F(Form("hExd_ws_T%i_CSI%i", i, j), Form("hExd_ws_T%i_CSI%i", i, j), 300, -5, 25);
    }
  }
  ...
  while (TreeReader->Next()) {
    ...
    if (cEntry % 100000 == 0) {
      ...
      std::cout << "\rEntry : " << cEntry << "/" << GetEntries << " --- " << Form("%.2f", 100. * cEntry / GetEntries)
                << " %"
                << " --- " << Form("%.00f RunEvt/sec", cEntry / Time) << " --- "
                << " Time Left : " << Form("%d min ", (int)TimeLeft / 60) << Form("%01.00d sec", (int)TimeLeft % 60)
                << std::flush;
    }
    ...
    if (TestEntry()) {
      ...
      if (M2_CsI_E_d.size() == 1 && mult == 1) {
        ...
        double OffX[4] = {0, 0, -10, -15};
        double OffY[4] = {0, 0, -10, 5};

        // Add energy Loss...
        if (CATSPhysics.PositionX.size() == 2 && CATSPhysics.PositionY.size() == 2) {
          TVector3 InteractionPosition(M2_X.at(0) + OffX[Must2Physics.TelescopeNumber[0] - 1],
                                       M2_Y.at(0) + OffY[Must2Physics.TelescopeNumber[0] - 1], M2_Z.at(0));
          ...
          if (CATSPhysics.PositionX.size() == 2) {
            ...
            if (CATSPhysics.DetNumber[0] == 1 && CATSPhysics.DetNumber[1] == 2) {
              ...
              PositionOnTargetX1 =
                  CATSPhysics.PositionX[0] - 2 + (CATSPhysics.PositionX[1] - CATSPhysics.PositionX[0] - 2) * t;
              ...
              // TVector3 BeamImpactCATS_s(CATSPhysics.PositionOnTargetX + 10, CATSPhysics.PositionOnTargetY + 5, 0);
              TVector3 BeamImpactCATS_s(CATSPhysics.PositionOnTargetX, CATSPhysics.PositionOnTargetY, 0);
              TVector3 BeamImpactCATS_NoCATS(0, 0, 0);
              TVector3 BeamDirection_CATS(CATSPhysics.PositionX[1] - CATSPhysics.PositionX[0],
                                          CATSPhysics.PositionY[1] - CATSPhysics.PositionY[0],
                                          CATSPhysics.PositionZ[1] - CATSPhysics.PositionZ[0]);
              // std::cout << BeamDirection_CATS.X() << " " << BeamDirection_CATS.Y() << " " << BeamDirection_CATS.Z()
              // << std::endl; TVector3 HitDirection_WithCATS = Must2Physics.GetPositionOfInteraction(0) -
              // BeamImpactCATS; TVector3 HitDirection_NoCATS = Must2Physics.GetPositionOfInteraction(0);
              ...
              // std::cout << ThetaNormalTarget_WithCATS << " " << ThetaNormalTarget_NoCATS << " " <<
              // CATSPhysics.PositionOnTargetX << " " << CATSPhysics.PositionOnTargetY<<  std::endl;
              if (cut_deuton->IsInside(M2_CsI_E_d.at(0), Must2Physics.Si_E[0]) && CATSPhysics.PositionX.size() == 2 &&
                  CATSPhysics.PositionY.size() == 2) {
                ...
                Ex_wb = Cr48_pd.ReconstructRelativistic(
                    deuteron_CH2.EvaluateInitialEnergy(ELab, TargetThickness * 0.5, ThetaNormalTarget_WithCATS),
                    HitDirection_WithCATS.Angle(BeamDirection_CATS));
                ...
                // TLorentzVector PHeavy_p3He = Reaction_pd->LorentzAfterReaction(Energy["alpha"] ,
                // M2_ThetaLab[countMust2]);
                ...
                for (unsigned int countExo = 0; countExo < EXO_AB_size; countExo++) {
                  ...
                  hExd_EGam->Fill(Ex_ws,
                                  Doppler_Correction(ExogamPhysics.Theta_D[countExo], ExogamPhysics.Phi_D[countExo], 0,
                                                     0, Beta_pd, ExogamPhysics.E_AB[countExo]));
                }
                ...
                hExd2_WithCATSs_m[Must2Physics.TelescopeNumber[0]]->Fill(ELab_ws, Ex_ws);
              ...
              if (cut_triton->IsInside(M2_CsI_E_t.at(0), Must2Physics.Si_E[0])) {
                ...
                ELab_t = Cr48_pt.ReconstructRelativistic(
                    triton_CH2.EvaluateInitialEnergy(ELab, TargetThickness * 0.5, ThetaNormalTarget_WithCATS),
                    HitDirection_WithCATS_s.Angle(BeamDirection));
                hExt->Fill(ELab_t);
  ...
  TFile* fout = new TFile("./data/DrawEx10_3_shifted.root", "RECREATE");
