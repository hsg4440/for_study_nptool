...
loop_geo() {
  ...
  for DIST in 2 3 4 5 7 10 15; do
    ...
    echo "/run/beamOn 1000000" >> $G4_MAC_FILE
    ...
    if [ $DIST -eq 15 ]; then
      ...
      nohup npsimulation -E $SOURCE_FILE -D $INPUT_File -O $OUT_DIR"/0Alpha1Electron_"$DIST"mm.root" -B $G4_MAC_FILE &
      sleep 30
    fi
  done
  ...
  for DIST in 2 3 4 5 7 10 15; do
    ...
    if [ $DIST -eq 15 ]; then
      ...
      nohup npsimulation -E $SOURCE_FILE -D $INPUT_File -O $OUT_DIR"/1Alpha1Electron_"$DIST"mm.root" -B $G4_MAC_FILE &
      sleep 30
    fi
  done
  ...
  for DIST in 2 3 4 5 7 10 15; do
    ...
    if [ $DIST -eq 15 ]; then
      ...
      nohup npsimulation -E $SOURCE_FILE -D $INPUT_File -O $OUT_DIR"/1Alpha2Electron_"$DIST"mm.root" -B $G4_MAC_FILE &
      sleep 30
    fi
  done
  ...
  for DIST in 2 3 4 5 7 10 15; do
    ...
    if [ $DIST -eq 15 ]; then
      ...
      nohup npsimulation -E $SOURCE_FILE -D $INPUT_File -O $OUT_DIR"/1Alpha3Electron_"$DIST"mm.root" -B $G4_MAC_FILE &
      sleep 30
    fi
  done
  ...
  for DIST in 2 3 4 5 7 10 15; do
    ...
    if [ $DIST -eq 15 ]; then
      ...
      nohup npsimulation -E $SOURCE_FILE -D $INPUT_File -O $OUT_DIR"/1Alpha4Electron_"$DIST"mm.root" -B $G4_MAC_FILE &
      sleep 30
    fi
  done
  ...
  for DIST in 2 3 4 5 7 10 15; do
    ...
    if [ $DIST -eq 15 ]; then
      ...
      nohup npsimulation -E $SOURCE_FILE -D $INPUT_File -O $OUT_DIR"/1Alpha5Electron_"$DIST"mm.root" -B $G4_MAC_FILE &
      sleep 30
    fi
  done
}
