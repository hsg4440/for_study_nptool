/* Predefine functions */
vector<vector<double>> GetExpDiffCross(double Energy, int MGX);
TH1F* PullThetaLabHist(int i, double minTheta, double gatesize, int MGX);
...
/* For summing seperate MGX figures together */
bool sumbool = 1;
...
////////////////////////////////////////////////////////////////////////////////
void CS(double Energy, double Spin, double spdf, double angmom, int MGX){
  ...
  auto file = new TFile("../SolidAngle_HistFile_15Aug22_ExcludeThePoor.root");
  cout << "OPENING FILE: ../SolidAngle_HistFile_15Aug22_ExcludeThePoor.root" << endl;
  string pullme = "SolidAngle_Lab_MG"+to_string(MGX);
  cout << "FINDING: " << pullme << endl;
  TH1F* SolidAngle = (TH1F*) file->FindObjectAny(pullme.c_str());
  ...
  vector<vector<double>> areaArray = GetExpDiffCross(means[indexE], MGX);
  delete c_PeakArea;
...
//cout << "binmin " << binmin << " to binmax " << binmax << endl;
    ...
    if(SA==0.0){
      //AoSA.push_back(NaN);
      //AoSAerr.push_back(0.0);
      cout << "excluding this angle for 0 solid angle..." << endl;
    } else {
...
*/
  ...
  auto gdSdO_copy = new TGraphErrors(*gdSdO);
  TCanvas* c_copy = new TCanvas("c_copy","c_copy",1000,1000);
  gdSdO_copy->SetTitle("dSigma over dOmega,  NOT scaled to theory");
  gdSdO_copy->Draw();

/*
  TList* collection = new TList();
  auto MergedMGX2 = new TGraphErrors();
  if(sumbool){
    for(int i=1; i<7; i++){
      string pullfile = "dSdO_1983_NoSlidingShift/dSdO_MG" + to_string(i) + ".root";
      cout << "OPENING FILE: " << pullfile << endl;
      auto file = new TFile(pullfile.c_str());
      cout << "FINDING GRAPH" << endl;
      TGraphErrors* gMGX = (TGraphErrors*) file->FindObjectAny("Graph");
      string newname = "dSdO_MG" + to_string(i);
      gMGX->SetName(newname.c_str());
      gMGX->SetTitle(newname.c_str());
      collection->Add(gMGX);
    }
    MergedMGX2->Merge(collection);
  }
*/

  /* Merged (averaged) by hand in 16Aug22_CompareSpecFactByMGX.ods */
/**/  double x[20] = {106.25,
108.75,
111.25,
113.75,
...
0.000281
  ...
  auto MergedMGX2 = new TGraphErrors(20,x,y,xerr,yerr);
/**/
  auto test = new TCanvas("test","test",1000,1000);
  MergedMGX2->Draw();
  ...
  TGraph* Final = new TGraph();
  if(sumbool){
    Final = FindNormalisation(TheoryDiffCross,MergedMGX2);
  }
  else{
...
vector<vector<double>> GetExpDiffCross(double Energy, int MGX){
  ...
  if(scaleTogether){
    if(MGX!=0){
    //TH1F* baseEx = PullThetaLabHist(0,firstAngle,widthAngleBins, MGX);
    //TH1F* basePS = PullPhaseSpaceHist(0,firstAngle,widthAngleBins);
    baseEx = PullThetaLabHist(0,firstAngle,widthAngleBins, MGX);
    basePS = PullPhaseSpaceHist(0,firstAngle,widthAngleBins);
    for(int i=1; i<numAngleBins;i++){
      TH1F* addEx = PullThetaLabHist(i,firstAngle,widthAngleBins, MGX); baseEx->Add(addEx,1.);
      TH1F* addPS = PullPhaseSpaceHist(i,firstAngle,widthAngleBins); basePS->Add(addPS,1.);
    }
} else { //Sum all MG's up!
    baseEx = PullThetaLabHist(0,firstAngle,widthAngleBins,1);
    basePS = PullPhaseSpaceHist(0,firstAngle,widthAngleBins);
    for(int i=1; i<numAngleBins;i++){
      TH1F* addEx = PullThetaLabHist(i,firstAngle,widthAngleBins, MGX+t); baseEx->Add(addEx,1.);
      TH1F* addPS = PullPhaseSpaceHist(i,firstAngle,widthAngleBins); basePS->Add(addPS,1.);
    }
    for(int t=2; t<7; t++){
      //TH1F* baseEx = PullThetaLabHist(0,firstAngle,widthAngleBins, MGX);
      //TH1F* basePS = PullPhaseSpaceHist(0,firstAngle,widthAngleBins);
      TH1F* baseExAdd = PullThetaLabHist(0,firstAngle,widthAngleBins, MGX+t);
      TH1F* basePSAdd = PullPhaseSpaceHist(0,firstAngle,widthAngleBins);
      for(int i=1; i<numAngleBins;i++){
        TH1F* addEx = PullThetaLabHist(i,firstAngle,widthAngleBins, MGX+t); baseExAdd->Add(addEx,1.);
        TH1F* addPS = PullPhaseSpaceHist(i,firstAngle,widthAngleBins); basePSAdd->Add(addPS,1.);
      }
      baseEx->Add(baseExAdd,1);
      basePS->Add(basePSAdd,1);
    }
    }
  ...
  for(int i=0; i<numAngleBins;i++){
    ...
    TH1F* gate = PullThetaLabHist(i,firstAngle,widthAngleBins, MGX);
...
////////////////////////////////////////////////////////////////////////////////
TH1F* PullThetaLabHist(int i, double minTheta, double gatesize, int MGX){
  string readme = "GateThetaLabHistograms_15Aug22_ExcludeThePoor_MG"+to_string(MGX)+".root";
  TFile* file = new TFile(readme.c_str(),"READ");
  ...
  TList *list = (TList*)file->Get("GateThetaLabHistograms_MGX");
